  ...
  async mExecute(pDiscordBot, message, args) {
    ...
    if (vTimeout - (Date.now() - vEconomy.Date) > 0) 
    {
      const vEmbed = new pDiscordBot.aDiscord.MessageEmbed()
        ...
        .setTitle("Erreur...")
        .setDescription(`Vous avez déjà récupéré votre paye du jour.\nVous pourrez recommencer dans ${vTime}.`)
    ...
    else
    {
      ...
      const vEmbed = new pDiscordBot.aDiscord.MessageEmbed()
        ...
        .setDescription(`${message.author}, vous avez touché ${vSalaire}₱ pour un solde total de ${vEconomy.Money}₱.\nVous pourrez recommencer dans ${ms(vTimeout - (Date.now() - vEconomy.Date))}.`)
        .setThumbnail(message.author.displayAvatarURL());
      message.reply(vEmbed);      
    }
    message.delete();
